<template>
    ...
    <!-- 타겟팅 목적 팝업 -->
    <b-modal
        v-model="isTargetGruopModalOpen"
        title="타겟팅 목적"
        hide-footer
        centered
        scrollable
...
const isTargetGruopModalOpen = ref(false) // 목적 list 모달 on/off
...
const showTargetGruopModal = async () => {
    ...
    isTargetGruopModalOpen.value = true
...
function updateExtra(key, field, rawValue) {
    // 1) recursive finder: completeList may contain nested group objects
    ...
    let node = null
    if (!found) {
        // fallback to targetConditionList (flat)
    ...
    // 2) ensure node.extraValues is linked to the shared extraValues map
    if (!node.extraValues) {
        node.extraValues =
            extraValues[key] ??
            (extraValues[key] = {
    ...
    // 4) mutate in-place to preserve references
    if (field === 'inputType') {
    ...
    // 5) trigger reactivity for top-level arrays if needed
    // If node is a direct child of completeList (top-level), replace to ensure reactivity
    if (listRef === completeList.value) {
        // find real index in parent completeList (may differ if nested)
...
const onSubQueryMyTargetSwitch = async () => {
    ...
    if (response) {
        console.log('selectTargetswitchSubQuery', response)
        subQueryList.value = response.targetSubQueryList
        total.value = response.total
    }
}
...
const dimensionSelectionTouched = reactive({}) // 모달에서 좌/우 이동 발생 여부를 기억
...
const showDimensionModal = async (item) => {
    ...
    currentDimensionItem.value = item || null
...
// 새 로직: 선택 리스트를 객체 그대로 저장/복원하여 페이징된 available 목록에 의존하지 않음
function initChosenForItemV2(item) {
    const key = item?.key
    if (!key) return
    if (!Array.isArray(chosenDimensionsMap[key])) {
        chosenDimensionsMap[key] = []
    }
    if (dimensionSelectionTouched[key]) {
        // 사용자가 좌/우 이동을 수행한 적이 있으면 현재 상태를 유지
        return
    }
    if (chosenDimensionsMap[key].length > 0) {
        // 이미 모달에서 선택을 진행 중이면 덮어쓰지 않고 유지
        return
    }

    const ex = extraValues[key] || {}
    const savedItems = Array.isArray(ex.dimensionItems)
        ? ex.dimensionItems
        : []
    ...
    if (savedItems.length > 0) {
        ...
        savedItems.forEach((d) =>
            uniq.set(String(d.metaDimensionId), { ...d })
        )
        chosenDimensionsMap[key] = Array.from(uniq.values())
        return
    }
...
function moveSelectedToChosen() {
    ...
    selectedAvailableIds.value = []
    dimensionSelectionTouched[key] = true
}
...
function moveAllToChosen() {
    ...
    const toAdd = availableDimensions.value || []
    toAdd.forEach((d) => {
        existing.set(String(d.metaDimensionId), d)
    })
    chosenDimensionsMap[key] = Array.from(existing.values())
    selectedAvailableIds.value = []
    dimensionSelectionTouched[key] = true
...
function confirmDimensionModal() {
    ...
    extraValues[key].dimensionItems = selected.map((d) => ({
        metaDimensionId: d.metaDimensionId,
        metaDimensionName: d.metaDimensionName
    }))
...
function resolveSourceNode(node) {
    // Robust: ignore __completeIndex assumptions (not top-level index),
    // locate by reference first, then by key recursively in completeList.
    try {
        if (!node) return null
