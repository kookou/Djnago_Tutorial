<template>
    <BContainer fluid>
        ...
        <template
            v-if="layoutType === 'horizontal' || layoutType === 'semibox'"
        >
...
export default {
    ...
    methods: {
        ...
        initActiveMenu() {
            ...
            // 2. 관리자 그룹 감지 (SubSidebar 전용)
            if (!matchingMenuItem && pathName.startsWith('/admin/')) {
                matchingMenuItem = items.find((x) =>
                    x.getAttribute('href')?.includes('sidebarAdmin')
                )

                // collapse 강제로 펼치기
                const adminCollapse = document.getElementById('sidebarAdmin')
                if (
                    adminCollapse &&
                    !adminCollapse.classList.contains('show')
                ) {
